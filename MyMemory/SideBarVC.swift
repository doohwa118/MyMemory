...
class SideBarVC: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.nameLabel.text = "얼렁뚱땅 개발자" // 기본 텍스트
        ...
        self.emailLabel.text = "20005170@naver.com" // 기본 텍스트
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0 { // 선택된 행이 새글 작성 메뉴일 때
            ...
            self.revealViewController()?.revealToggle(self) // 사이드 바 닫음
        } else if indexPath.row == 5 { // 계정 관리
            let uv = self.storyboard?.instantiateViewController(withIdentifier: "_Profile")
            self.present(uv!, animated: true) { // 트레일링 클로저
                self.revealViewController()?.revealToggle(self) // 사이드 바 닫음
            }
        }
    }
